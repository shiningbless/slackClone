...
class Group extends Model {
  ...
  const Group._internal({required this.id, required name}) : _name = name;

  factory Group({String? id, required String name}) {
    return Group._internal(id: id == null ? UUID.getUUID() : id, name: name);
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is Group && id == other.id && _name == other._name;
  }
  ...
  static var schema = Model.defineSchema(define: (ModelSchemaDefinition modelSchemaDefinition) {
    ...
    modelSchemaDefinition.authRules = [
      AuthRule(
          authStrategy: AuthStrategy.GROUPS, groupClaim: "cognito:groups", groups: ["admin"], operations: [ModelOperation.CREATE, ModelOperation.READ, ModelOperation.UPDATE, ModelOperation.DELETE]),
      AuthRule(authStrategy: AuthStrategy.PRIVATE, operations: [ModelOperation.READ])
    ];

    modelSchemaDefinition.addField(ModelFieldDefinition.id());

    modelSchemaDefinition.addField(ModelFieldDefinition.field(key: Group.NAME, isRequired: true, ofType: ModelFieldType(ModelFieldTypeEnum.string)));
  });
}
